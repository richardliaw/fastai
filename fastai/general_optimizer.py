...
__all__ = [
    'StatScope', 'Statistic', 'ConstStatistic', 'AvgStatistic', 'AvgSquare',
    'GeneralOptimizer'
]
...
class AvgStatistic(Statistic):
    ...
    def update(self, state, param, val=None, step=None):
        ...
        # For everything else, `state` is a scalar
            ...
            res = state * param + self._get_val1(val) * (1 - param
                                                         if self.decay else 1.)
        elif self.count != 0:
            res = state * param + self.val / self.count * (1 - param if
                                                           self.decay else 1.)
        else:
            return state
...
class AvgSquare(AvgStatistic):
    ...
    def _get_val2(self, state, val, param):
        return state.addcmul_(1 - param, val,
                              val) if self.decay else state.addcmul_(val, val)

    def _get_val3(self, state, val, param):
        v = val.view(val.size(0), -1).mean(1)
        return state.addcmul_(1 - param, v,
                              v) if self.decay else state.addcmul_(v, v)
...
class GeneralOptimizer(Optimizer):
    def __init__(self, params, stats=None, on_step: Callable = None):
        defaults = {
            s.name: s.param
            for s in listify(stats) if s.name is not None
        }
        super().__init__(params, defaults)
        self.global_stats, self.group_stats, self.layer_stats, self.channel_stats, self.weight_stats = self._split_stats(
            stats)
        self.init_stats()
    ...
    def _split_stats(self, stats):
        splits = [[stat for stat in listify(stats) if stat.scope == scope]
    ...
    def _init_stats(self, stats, data=None):
        return {
            stat.buf: stat.init
            if data is None else torch.zeros_like(data) + stat.init
            for stat in stats if stat.buf is not None
        }
    ...
    def init_stats(self):
        ...
        for i, pg in enumerate(self.param_groups):
            ...
            for p in pg['params']:
                ...
                self.state[p].update(
                    self._init_stats(self.channel_stats,
                                     p.data.view(p.data.size(0), -1).mean(1)))
                self.state[p].update(
                    self._init_stats(self.weight_stats, p.data))
        ...
        for i, pg in enumerate(self.param_groups):
            ...
            for p in pg['params']:
                if p.grad is not None:
                    ...
                    self._set_bufs(
                        p, self.layer_stats + self.channel_stats +
                        self.weight_stats, pg, p.grad.data)
