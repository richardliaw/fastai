...
__all__ = [
    'URLs', 'Config', 'untar_data', 'download_data', 'datapath4file',
    'url2name', 'url2path'
]
...
_checks = {
    ...
    URLs.AMAZON_REVIEWS_POLARITY: (688339454,
    ...
    URLs.YELP_REVIEWS_POLARITY: (166373201,
...
def url2path(url, data=True, ext: str = '.tgz'):
    ...
    name = url2name(url)
    return datapath4file(
        name, ext=ext, archive=False) if data else modelpath4file(
            name, ext=ext)


def _url2tgz(url, data=True, ext: str = '.tgz'):
    return datapath4file(
        f'{url2name(url)}{ext}', ext=ext) if data else modelpath4file(
            f'{url2name(url)}{ext}', ext=ext)
...
def untar_data(url: str,
    ...
    if force_download or (fname.exists() and url in _checks
                          and _check_file(fname) != _checks[url]):
        print(
