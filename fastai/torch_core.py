...
bias_types = (nn.Linear, nn.Conv1d, nn.Conv2d, nn.Conv3d, nn.ConvTranspose1d,
...
def _new_torch_cuda_set_device(device):
    ...
    defaults.device = torch.device('cuda', device) if isinstance(
        device, int) else device
...
def get_param_groups(
        layer_groups: Collection[nn.Module]) -> List[List[nn.Parameter]]:
    return [
        sum([list(trainable_params(c)) for c in l.children()], [])
        for l in layer_groups
    ]


def split_no_wd_params(
        layer_groups: Collection[nn.Module]) -> List[List[nn.Parameter]]:
...
def init_default(m: nn.Module,
                 func: LayerFunc = nn.init.kaiming_normal_) -> None:
...
class NoneReduceOnCPU():
    ...
    def __enter__(self):
        if hasattr(self.loss_func,
                   'weight') and self.loss_func.weight is not None:
...
def model_type(dtype):
    ...
    return (torch.float32 if np.issubdtype(dtype, np.floating) else torch.int64
...
def uniform(low: Number, high: Number = None,
            size: Optional[List[int]] = None) -> FloatOrTensor:
    "Draw 1 or shape=`size` random floats from uniform dist: min=`low`, max=`high`."
    if high is None: high = low
    return random.uniform(
        low,
        high) if size is None else torch.FloatTensor(*listify(size)).uniform_(
            low, high)
...
def uniform_int(low: int, high: int,
                size: Optional[List[int]] = None) -> IntOrTensor:
    "Generate int or tensor `size` of ints between `low` and `high` (included)."
    return random.randint(low, high) if size is None else torch.randint(
        low, high + 1, size)
...
def try_int(o: Any) -> Any:
    ...
    if isinstance(o, collections.Sized) or getattr(o, '__array_interface__',
                                                   False):
        return o
    try:
        return int(o)
...
def get_model(model: nn.Module):
    ...
    return model.module if isinstance(
        model, (DistributedDataParallel, nn.DataParallel)) else model
...
def add_metrics(last_metrics: Collection[Rank0Tensor],
