...
__all__ = [
    'error_rate', 'accuracy', 'accuracy_thresh', 'dice', 'exp_rmspe', 'fbeta',
    'FBeta', 'mse', 'mean_squared_error', 'mae', 'mean_absolute_error', 'rmse',
    'root_mean_squared_error', 'msle', 'mean_squared_logarithmic_error',
    'explained_variance', 'r2_score', 'top_k_accuracy', 'KappaScore',
    'ConfusionMatrix', 'MatthewsCorreff', 'Precision', 'Recall', 'R2Score',
    'ExplainedVariance', 'ExpRMSPE', 'RMSE', 'Perplexity', 'AUROC',
    'auc_roc_score', 'roc_curve', 'MultiLabelFbeta', 'foreground_acc'
]


def fbeta(y_pred: Tensor,
          y_true: Tensor,
          thresh: float = 0.2,
          beta: float = 2,
          eps: float = 1e-9,
          sigmoid: bool = True) -> Rank0Tensor:
...
def dice(input: Tensor, targs: Tensor, iou: bool = False) -> Rank0Tensor:
    ...
    if not iou:
        return (2. * intersect / union
                if union > 0 else union.new([1.]).squeeze())
    else:
...
class RegMetrics(Callback):
    ...
    def on_batch_end(self, last_output: Tensor, last_target: Tensor, **kwargs):
        assert last_output.numel() == last_target.numel(
...
class ExplainedVariance(RegMetrics):
    ...
    def on_epoch_end(self, last_metrics, **kwargs):
        return add_metrics(last_metrics,
                           explained_variance(self.preds, self.targs))
...
class RMSE(RegMetrics):
    ...
    def on_epoch_end(self, last_metrics, **kwargs):
        return add_metrics(last_metrics,
                           root_mean_squared_error(self.preds, self.targs))
...
class ConfusionMatrix(Callback):
    ...
    def on_batch_end(self, last_output: Tensor, last_target: Tensor, **kwargs):
        ...
        cm = ((preds == self.x[:, None]) &
              (targs == self.x[:, None, None])).sum(
...
class CMScores(ConfusionMatrix):
    ...
    average: Optional[
        str] = "binary"  # `binary`, `micro`, `macro`, `weigthed` or None
...
class KappaScore(ConfusionMatrix):
    ...
    def on_epoch_end(self, last_metrics, **kwargs):
        ...
        if self.weights is None:
            ...
            w = torch.abs(w - torch.t(w)) if self.weights == "linear" else (
                w - torch.t(w))**2
        else:
            raise ValueError(
...
class MatthewsCorreff(ConfusionMatrix):
    ...
    def on_epoch_end(self, last_metrics, **kwargs):
        ...
        return add_metrics(last_metrics,
                           cov_ytyp / torch.sqrt(cov_ytyt * cov_ypyp))
...
class Perplexity(Callback):
    ...
    def on_batch_end(self, last_output, last_target, **kwargs):
        self.loss += last_target.size(1) * CrossEntropyFlat()(last_output,
                                                              last_target)
...
def roc_curve(input: Tensor, targ: Tensor):
    ...
    threshold_idxs = torch.cat((distinct_value_indices,
                                LongTensor([len(targ) - 1]).to(targ.device)))
...
class MultiLabelFbeta(LearnerCallback):
    ...
    def on_train_begin(self, **kwargs):
        ...
        if self.average != "none":
            self.learn.recorder.add_metric_names([f'{self.average}_fbeta'])
        else:
            self.learn.recorder.add_metric_names(
                [f"fbeta_{c}" for c in self.learn.data.classes])
    ...
    def on_batch_end(self, last_output, last_target, **kwargs):
        pred, targ = (last_output.sigmoid() if self.sigmoid else
                      last_output) > self.thresh, last_target.byte()
        m = pred * targ
    ...
    def fbeta_score(self, precision, recall):
        return (1 + self.beta2) * (precision * recall) / (
    ...
    def on_epoch_end(self, last_metrics, **kwargs):
        ...
        if self.average == "micro":
            precision, recall = self.tp.sum() / self.total_pred.sum(
            ), self.tp.sum() / self.total_targ.sum()
            res = self.fbeta_score(precision, recall)
        elif self.average == "macro":
            res = self.fbeta_score((self.tp / self.total_pred),
                                   (self.tp / self.total_targ)).mean()
        elif self.average == "weighted":
            scores = self.fbeta_score((self.tp / self.total_pred),
                                      (self.tp / self.total_targ))
            res = (scores * self.total_targ).sum() / self.total_targ.sum()
        elif self.average == "none":
            res = listify(
                self.fbeta_score((self.tp / self.total_pred),
                                 (self.tp / self.total_targ)))
        else:
            raise Exception(
