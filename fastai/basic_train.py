...
__all__ = [
    ...
    'loss_batch', 'train_epoch', 'validate', 'get_preds', 'load_learner'
]
...
def get_preds(model: nn.Module,
              dl: DataLoader,
              pbar: Optional[PBar] = None,
              cb_handler: Optional[CallbackHandler] = None,
              activ: nn.Module = None,
              loss_func: OptLossFunc = None,
              n_batch: Optional[int] = None) -> List[Tensor]:
    "Tuple of predictions and targets, and optional losses (if `loss_func`) using `dl`, max batches `n_batch`."
    res = [
        torch.cat(o).cpu() for o in zip(*validate(
            model,
            dl,
...
def validate(model: nn.Module,
    ...
    with torch.no_grad():
        ...
        for xb, yb in progress_bar(dl, parent=pbar, leave=(pbar is not None)):
            ...
            val_loss = loss_batch(
                model, xb, yb, loss_func, cb_handler=cb_handler)
...
def train_epoch(model: nn.Module, dl: DataLoader, opt: optim.Optimizer,
...
def fit(epochs: int,
    ...
    assert len(
        learn.data.train_dl
    ) != 0, f"""Your training dataloader is empty, can't train a model.
    ...
    try:
        for epoch in pbar:
            ...
            for xb, yb in progress_bar(learn.data.train_dl, parent=pbar):
                ...
                loss = loss_batch(learn.model, xb, yb, learn.loss_func,
...
def _loss_func2activ(loss_func):
    ...
    if cls_name in loss_func_name2activ:
        if cls_name == 'poisson_nll_loss' and (not getattr(
                loss_func, 'log_input', True)):
            return
...
class Learner():
    ...
    def fit(self,
        ...
        callbacks = [
            cb(self)
            for cb in self.callback_fns + listify(defaults.extra_callback_fns)
        ] + listify(callbacks)
        if defaults.extra_callbacks is not None:
            callbacks += defaults.extra_callbacks
        fit(epochs,
            self,
    ...
    def export(self,
        ...
        args = [
            'opt_func', 'loss_func', 'metrics', 'true_wd', 'bn_wd', 'wd',
            'train_bn', 'model_dir', 'callback_fns'
        ]
        state = {a: getattr(self, a) for a in args}
        state['cb_state'] = {
            cb.__class__: cb.get_state()
        ...
        with ModelOnCPU(self.model) as m:
            state['model'] = m
            xtra = dict(normalize=self.data.norm.keywords) if getattr(
                self.data, 'norm', False) else {}
    ...
    def save(self,
        ...
        if rank_distrib(): return  # don't save if slave proc
        target = self.path / self.model_dir / f'{file}.pth' if is_pathlike(
            file) else file
    ...
    def load(self,
        ...
        source = self.path / self.model_dir / f'{file}.pth' if is_pathlike(
            file) else file
    ...
    def purge(self, clear_opt: bool = True):
        ...
        attrs_pkl = [
            'bn_wd', 'callback_fns', 'layer_groups', 'loss_func', 'metrics',
            'model', 'model_dir', 'opt_func', 'path', 'train_bn', 'true_wd',
            'wd'
        ]
        ...
        self.callbacks = [
            load_callback(c, s, self) for c, s in cb_state.items()
        ]
    ...
    def get_preds(self,
        ...
        return get_preds(
            ...
            activ=_loss_func2activ(self.loss_func),
            loss_func=lf,
            n_batch=n_batch,
            pbar=pbar)
    ...
    def predict(self,
        ...
        y = ds.y.reconstruct(pred, x) if has_arg(
            ds.y.reconstruct, 'x') else ds.y.reconstruct(pred)
...
class Recorder(LearnerCallback):
    ...
    def on_train_begin(self, pbar: PBar, metrics_names: Collection[str],
                       **kwargs: Any) -> None:
        "Initialize recording status at beginning of training."
        self.pbar = pbar
        self.names = ['epoch', 'train_loss'] if self.no_val else [
    ...
    def plot(self,
        ...
        if suggestion:
            ...
            ax.plot(
                lrs[mg], losses[mg], markersize=10, marker='o', color='red')
    ...
    def plot_losses(self,
        ...
        fig, ax = plt.subplots(1, 1)
        losses = self._split_list(self.losses, skip_start, skip_end)
        iterations = self._split_list(
            range_of(self.losses), skip_start, skip_end)
        ax.plot(iterations, losses, label='Train')
        val_iter = self._split_list_val(
            np.cumsum(self.nb_batches), skip_start, skip_end)
        val_losses = self._split_list_val(self.val_losses, skip_start,
                                          skip_end)
    ...
    def _split_list(self, vals: Collection[float], skip_start: int,
                    skip_end: int):
        return vals[skip_start:-skip_end] if skip_end > 0 else vals[
            skip_start:]

    def _split_list_val(self, vals: Collection[float], skip_start: int,
                        skip_end: int):
...
def load_callback(class_func, state, learn: Learner):
    ...
    res = class_func(learn, **init_kwargs) if issubclass(
        class_func, LearnerCallback) else class_func(**init_kwargs)
    for k, v in others.items():
        setattr(res, k, v)
    return res


def load_learner(path: PathOrStr,
                 file: PathLikeOrBinaryStream = 'export.pkl',
                 test: ItemList = None,
                 **db_kwargs):
    ...
    state = torch.load(
        source, map_location='cpu') if defaults.device == torch.device(
            'cpu') else torch.load(source)
