...
def random_split(valid_pct: float, *arrs: NPArrayableList) -> SplitArrayList:
    ...
    assert (valid_pct >= 0 and valid_pct <= 1
...
def recurse_eq(arr1, arr2):
    if is_listy(arr1):
        return is_listy(arr2) and len(arr1) == len(arr2) and np.all(
            [recurse_eq(x, y) for x, y in zip(arr1, arr2)])
    else:
...
def join_paths(fnames: FilePathList,
               path: PathOrStr = '.') -> Collection[Path]:
...
def array(a, dtype: type = None, **kwargs) -> np.ndarray:
    ...
    if not isinstance(a, collections.Sized) and not getattr(
            a, '__array_interface__', False):
        a = list(a)
    if np.int_ == np.int32 and dtype is None and is_listy(a) and len(
...
def parallel(func, arr: Collection, max_workers: int = None):
    ...
    if max_workers < 2:
        results = [
            func(o, i)
            for i, o in progress_bar(enumerate(arr), total=len(arr))
        ]
    else:
        with ProcessPoolExecutor(max_workers=max_workers) as ex:
            ...
            for f in progress_bar(
                    concurrent.futures.as_completed(futures), total=len(arr)):
