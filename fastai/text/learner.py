...
__all__ = [
    'RNNLearner', 'LanguageLearner', 'convert_weights', 'decode_spec_tokens',
    'get_language_model', 'language_model_learner', 'MultiBatchEncoder',
    'get_text_classifier', 'text_classifier_learner', 'PoolingLinearClassifier'
]
...
def convert_weights(wgts: Weights, stoi_wgts: Dict[str, int],
                    itos_new: Collection[str]) -> Weights:
    "Convert the model `wgts` to go with a new vocabulary."
    dec_bias, enc_wgts = wgts.get('1.decoder.bias',
                                  None), wgts['0.encoder.weight']
...
class RNNLearner(Learner):
    ...
    def __init__(self,
                 ...
                 metrics=None,
                 **learn_kwargs):
        is_class = (hasattr(data.train_ds, 'y')
                    and (isinstance(data.train_ds.y, CategoryList)
    ...
    def save_encoder(self, name: str):
        ...
        torch.save(encoder.state_dict(),
                   self.path / self.model_dir / f'{name}.pth')
    ...
    def load_encoder(self, name: str, device: torch.device = None):
        ...
        encoder.load_state_dict(
            torch.load(self.path / self.model_dir / f'{name}.pth'))
        encoder.load_state_dict(
            torch.load(
                self.path / self.model_dir / f'{name}.pth',
                map_location=device))
    ...
    def load_pretrained(self,
        ...
        wgts = torch.load(
            wgts_fname, map_location=lambda storage, loc: storage)
    ...
    def get_preds(self,
        ...
        preds = super().get_preds(
            ds_type=ds_type, with_loss=with_loss, n_batch=n_batch, pbar=pbar)
...
class LanguageLearner(RNNLearner):
    ...
    def predict(self,
        ...
        return text + sep + sep.join(
            decoder(self.data.vocab.textify(new_idx, sep=None)))
...
def get_language_model(arch: Callable,
                       vocab_sz: int,
                       config: dict = None,
                       drop_mult: float = 1.):
...
def language_model_learner(data: DataBunch,
                           arch,
                           config: dict = None,
                           drop_mult: float = 1.,
...
class PoolingLinearClassifier(nn.Module):
    ...
    def __init__(self, layers: Collection[int], drops: Collection[float]):
        ...
        if len(drops) != len(layers) - 1:
            raise ValueError(
                "Number of layers and dropout values do not match.")
        activs = [nn.ReLU(inplace=True)] * (len(layers) - 2) + [None]
        for n_in, n_out, p, actn in zip(layers[:-1], layers[1:], drops,
                                        activs):
    ...
    def forward(self, input: Tuple[Tensor, Tensor, Tensor]
        ...
        avg_pool *= output.size(1) / (
            output.size(1) - mask.type(avg_pool.dtype).sum(dim=1))[:, None]
        max_pool = output.masked_fill(mask[:, :, None],
                                      -float('inf')).max(dim=1)[0]
...
class MultiBatchEncoder(nn.Module):
    ...
    def concat(self, arrs: Collection[Tensor]) -> Tensor:
        ...
        return [
            torch.cat([l[si] for l in arrs], dim=1) for si in range_of(arrs[0])
        ]
    ...
    def forward(self, input: LongTensor) -> Tuple[Tensor, Tensor]:
        ...
        for i in range(0, sl, self.bptt):
            ...
            if i > (sl - self.max_len):
                masks.append(input[:, i:min(i +
                                            self.bptt, sl)] == self.pad_idx)
                raw_outputs.append(r)
                outputs.append(o)
        return self.concat(raw_outputs), self.concat(outputs), torch.cat(
            masks, dim=1)


def get_text_classifier(arch: Callable,
                        vocab_sz: int,
                        n_class: int,
                        bptt: int = 70,
                        max_len: int = 20 * 70,
...
def text_classifier_learner(data: DataBunch,
                            arch: Callable,
                            bptt: int = 70,
                            max_len: int = 70 * 20,
                            config: dict = None,
    ...
    meta = _model_meta[arch]
    learn = RNNLearner(
        data, model, split_func=meta['split_clas'], **learn_kwargs)
    if pretrained:
        if 'url' not in meta:
            warn("There are no pretrained weights for that architecture yet!")
            return learn
