...
__all__ = [
    'Activation', 'PositionalEncoding', 'GeLU', 'Swish', 'feed_forward',
    'MultiHeadAttention', 'MultiHeadRelativeAttention', 'DecoderLayer',
    'Transformer', 'TransformerXL', 'tfmer_lm_config', 'tfmer_clas_config',
    'tfmer_lm_split', 'tfmer_clas_split', 'tfmerXL_lm_config',
    'tfmerXL_clas_config', 'tfmerXL_lm_split', 'tfmerXL_clas_split'
]
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x: Tensor, mask: Tensor = None, **kwargs):
        return self.ln(x + self.drop_res(
            self.out(self._apply_attention(x, mask=mask, **kwargs))))

    def _apply_attention(self, x: Tensor, mask: Tensor = None):
        bs, x_len = x.size(0), x.size(1)
        wq, wk, wv = torch.chunk(self.attention(x), 3, dim=-1)
        wq, wk, wv = map(
            lambda x: x.view(bs, x.size(1), self.n_heads, self.d_head),
            (wq, wk, wv))
        wq, wk, wv = wq.permute(0, 2, 1, 3), wk.permute(0, 2, 3,
                                                        1), wv.permute(
                                                            0, 2, 1, 3)
        ...
        return attn_vec.permute(0, 2, 1, 3).contiguous().contiguous().view(
            bs, x_len, -1)
...
class Transformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.pos_enc = nn.Embedding(
            ctx_len,
            d_model) if learned_pos_enc else PositionalEncoding(d_model)
    ...
    def forward(self, x):
        ...
        inp = self.drop_emb(
            self.encoder(x) +
            self.pos_enc(pos)[None])  #.mul_(self.d_model ** 0.5)
        mask = torch.triu(
            x.new_ones(x_len, x_len),
            diagonal=1).byte()[None, None] if self.mask else None
...
class TransformerXL(nn.Module):
    ...
    def __init__(self,
        ...
        self.u = nn.Parameter(torch.Tensor(
            n_heads, 1,
            d_head))  #Remove 1 for einsum implementation of attention
    ...
    def reset(self):
        ...
        self.hidden = [
            next(self.parameters()).data.new(0)
    ...
    def forward(self, x):
        ...
        m_len = self.hidden[0].size(1) if hasattr(
            self, 'hidden') and len(self.hidden[0].size()) > 1 else 0
        seq_len = m_len + x_len
        mask = torch.triu(
            x.new_ones(x_len, seq_len),
            diagonal=1 + m_len).byte()[None, None] if self.mask else None
        #[None,:,:None] for einsum implementation of attention
        hids = []
        pos = torch.arange(
...
def tfmerXL_lm_split(model: nn.Module) -> List[nn.Module]:
    ...
    groups = [
        list(encoder.layers[:n]) +
        [ParameterModule(encoder.u),
         ParameterModule(encoder.v)]
    ]
    return groups + [
        list(encoder.layers[n:2 * n]),
        list(encoder.layers[2 * n:]), [encoder.encoder, model[1]]
    ]
...
def tfmerXL_clas_split(model: nn.Module) -> List[nn.Module]:
    ...
    return groups + [
        list(encoder.layers[n:2 * n]),
        list(encoder.layers[2 * n:]), [model[1]]
    ]
