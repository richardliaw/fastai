...
class ForgetMultGPU(Function):
    ...
    def backward(ctx, grad_output):
        ...
        return (grad_x, grad_f, (None
                                 if hidden_init is None else grad_h), None)
...
def forget_mult_CPU(x: Tensor,
    ...
    prev_h = None if hidden_init is None else hidden_init.unsqueeze(
        1 if batch_first else 0)
    idx_range = range(len(inputs) - 1, -1, -1) if backward else range(
        len(inputs))
    for i in idx_range:
        prev_h = inputs[i] * forgets[i] if prev_h is None else inputs[i] * forgets[i] + (
            1 - forgets[i]) * prev_h
...
class QRNNLayer(nn.Module):
    ...
    def forward(self, inp, hid=None):
        ...
        if self.zoneout and self.training:
            mask = dropout_mask(f_gate, f_gate.size(),
                                self.zoneout).requires_grad_(False)
            f_gate = f_gate * mask
        ...
        if self.window > 1 and self.save_prev_x:
            if self.backward:
                self.prevX = (inp[:, :1]
                              if self.batch_first else inp[:1]).detach()
            else:
                self.prevX = (inp[:, -1:]
                              if self.batch_first else inp[-1:]).detach()
    ...
    def _get_source(self, inp):
        ...
        inp_shift = [
            ...
            if self.prevX is None else self.prevX
        ]
        if self.backward:
...
class QRNN(nn.Module):
    ...
    def __init__(self,
                 ...
                 window: int = None,
                 output_gate: bool = True):
        assert not (save_prev_x and bidirectional
                    ), "Can't save the previous X with bidirectional."
        assert bias == True, 'Removing underlying bias is not yet supported'
        super().__init__()
        kwargs = dict(
            batch_first=batch_first, zoneout=zoneout, output_gate=output_gate)
    ...
    def forward(self, inp, hid=None):
        ...
        for i, layer in enumerate(self.layers):
            inp, h = layer(
                inp, None
                if hid is None else hid[2 * i if self.bidirectional else i])
            new_hid.append(h)
            if self.bidirectional:
                inp_bwd, h_bwd = self.layers_bwd[i](inp_bwd, None
                                                    if hid is None else
                                                    hid[2 * i + 1])
