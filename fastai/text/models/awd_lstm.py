...
__all__ = [
    'EmbeddingDropout', 'LinearDecoder', 'AWD_LSTM', 'RNNDropout',
    'SequentialRNN', 'WeightDropout', 'dropout_mask', 'awd_lstm_lm_split',
...
class EmbeddingDropout(nn.Module):
    ...
    def forward(self, words: LongTensor,
...
class AWD_LSTM(nn.Module):
    ...
    def forward(self, input: Tensor,
                from_embeddings: bool = False) -> Tuple[Tensor, Tensor]:
        if from_embeddings: bs, sl, es = input.size()
        else: bs, sl = input.size()
        if bs != self.bs:
            self.bs = bs
            self.reset()
        raw_output = self.input_dp(
            input if from_embeddings else self.encoder_dp(input))
...
def value2rgba(x: float, cmap: Callable = cm.RdYlGn,
               alpha_mult: float = 1.0) -> Tuple:
...
def piece_attn_html(pieces: List[str],
                    attns: List[float],
                    sep: str = ' ',
                    **kwargs) -> str:
...
class TextClassificationInterpretation(ClassificationInterpretation):
    ...
    def __init__(self,
        ...
        super(TextClassificationInterpretation, self).__init__(
            learn, probs, y_true, losses, ds_type)
    ...
    def intrinsic_attention(self, text: str, class_id: int = None):
        ...
        self.model.eval()
        cl = self.model[1](lstm_output +
                           (torch.zeros_like(ids).byte(), ))[0].softmax(dim=-1)
    ...
    def html_intrinsic_attention(self,
                                 text: str,
                                 class_id: int = None,
                                 **kwargs) -> str:
    ...
    def show_intrinsic_attention(self,
                                 text: str,
                                 class_id: int = None,
                                 **kwargs) -> None:
        text, attn = self.intrinsic_attention(text, class_id)
    ...
    def show_top_losses(self, k: int, max_len: int = 70) -> None:
        ...
        for i, idx in enumerate(tl_idx):
            ...
            txt = ' '.join(tx.text.split(' ')
                           [:max_len]) if max_len is not None else tx.text
            tmp = [
                txt, f'{classes[self.pred_class[idx]]}', f'{classes[cl]}',
                f'{self.losses[idx]:.2f}', f'{self.probs[idx][cl]:.2f}'
            ]
            items.append(tmp)
