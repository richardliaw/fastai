...
__all__ = [
    'show_test', 'doctest', 'find_related_tests', 'lookup_db',
    'find_test_matches', 'find_test_files', 'fuzzy_test_match',
    'get_pytest_html'
]
...
def get_tests_dir(elt) -> Path:
    ...
    if not test_dir.exists():
        raise OSError('Could not find test directory at this location:',
                      test_dir)
    return test_dir
...
def find_test_files(elt, exact_match: bool = False) -> List[Path]:
    ...
    matches = [
        test_dir / o.name for o in os.scandir(test_dir)
        if _is_file_match(elt, o.name)
    ]
...
def _is_file_match(elt, file_name: str, exact_match: bool = False) -> bool:
    ...
    return re.match(f'test_{subdir}\w*{Path(fp).stem}{exact_re}\.py',
                    file_name)
...
def fuzzy_test_match(fn_name: str, lines: List[Dict],
                     rel_path: str) -> List[TestFunctionMatch]:
