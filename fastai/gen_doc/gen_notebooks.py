...
__all__ = [
    'create_module_page', 'update_module_page', 'import_mod', 'link_nb',
    'update_notebooks', 'generate_missing_metadata', 'update_nb_metadata'
]
    ...
    for node in ast.walk(ast.parse(fstr)):
        if isinstance(node, ast.Assign) and hasattr(node.targets[0], 'id'):
            ...
            if match and match.group(
                    1) != '__all__':  # only top level assignment
...
class ExecuteShowDocPreprocessor(ExecutePreprocessor):
    ...
    def preprocess_cell(self, cell, resources, index):
        if 'source' in cell and cell.cell_type == "code":
            if IMPORT_RE.search(cell['source']) or SHOW_DOC_RE.search(
                    cell['source']):
...
_default_exclude = [
    '.ipynb_checkpoints', '__pycache__', '__init__.py', 'imports'
]
...
def get_module_names(path_dir, exclude=None):
    ...
    files = sorted(
        path_dir.glob('*'), key=lambda x: (x.is_dir(), x.name),
        reverse=True)  # directories first
    res = [f'{path_dir.name}']
    for f in files:
...
def update_nb_metadata(nb_path=None,
    ...
    data = {k: v
            for (k, v) in data.items() if v is not None}  # remove none values
    if not data: return
...
def get_imported_modules(cells, nb_module_name=''):
    ...
    nb_imports = [
        match.group(1) for cell in cells
...
def get_top_level_modules(num_levels=1):
    ...
    filtered_n = filter(lambda x: x.count('.') <= num_levels,
                        get_module_names(mod_dir))
    return sorted(
        filtered_n, key=lambda s: s.count('.'),
        reverse=True)  # Submodules first (sorted by periods)
...
def check_nbconvert_version():
    ...
    assert nbconvert.version_info >= (
        5, 4,
        0), "Please update nbconvert to >=5.4 for consistent .html output"


def update_notebooks(source_path,
                     dest_path=None,
                     update_html=True,
    ...
    if source_path.is_file():
        dest_path = source_path.parent if dest_path is None else Path(
            dest_path)
        html_path = dest_path / '..' / 'docs' if html_path is None else Path(
            html_path)
