...
__all__ = [
    'get_fn_link', 'link_docstring', 'show_doc', 'get_ft_names', 'md2html',
    'get_exports', 'show_video', 'show_video_from_youtube', 'import_mod',
    'get_source_link', 'is_enum', 'jekyll_note', 'jekyll_warn',
    'jekyll_important', 'doc'
]
...
_typing_names = {
    t: n
    for t, n in fastai_types.items() if t.__module__ == 'typing'
}
...
def partial_repr(t):
    args = (t.func, ) + t.args + tuple(
        [f'{k}={v}' for k, v in t.keywords.items()])
...
def format_param(p):
    ...
    arg_prefix = arg_prefixes.get(p.kind,
...
def format_ft_def(func, full_name: str = None) -> str:
    ...
    fmt_params = [
        format_param(param) for name, param in sig.parameters.items()
...
def show_doc(elt,
    ...
    test_link, test_modal = get_pytest_html(
        elt, anchor_id=anchor_id) if show_tests else ('', '')
...
def md2html(md):
    if nbconvert.__version__ < '5.5.0': return HTMLExporter().markdown2html(md)
        ...
        return HTMLExporter().markdown2html(
            defaultdict(lambda: defaultdict(dict)), md)
...
def format_docstring(elt,
                     arg_comments: dict = {},
                     alt_doc_string: str = '',
                     ignore_warn: bool = False) -> str:
    ...
    args = inspect.getfullargspec(elt).args if not is_enum(
        elt.__class__) else elt.__members__.keys()
...
def find_elt(modvars, keyword, match_last=False):
    ...
    if hasattr(comp_elt, '__dict__'):
        return find_elt(
            comp_elt.__dict__, '.'.join(comps[1:]), match_last=match_last)
...
def show_doc_from_name(mod_name,
                       ft_name: str,
                       doc_string: bool = True,
                       arg_comments: dict = {},
                       alt_doc_string: str = ''):
    ...
    for elt_name in get_exports(mod):
        ...
        if mod.__file__.endswith('__init__.py'):
            ...
            if inspect.isclass(elt) or inspect.isfunction(elt):
                ...
                continue
        if include_inner and inspect.isclass(elt) and not is_enum(
                elt.__class__):
...
def get_pytorch_link(ft) -> str:
    ...
    if name.startswith('torch.nn') and inspect.ismodule(
            ft):  # nn.functional is special case
