...
__all__ = [
    'ItemList', 'CategoryList', 'MultiCategoryList', 'MultiCategoryProcessor',
    'LabelList', 'ItemLists', 'get_files', 'PreProcessor', 'LabelLists',
    'FloatList', 'CategoryProcessor', 'EmptyLabelList', 'MixedItem',
    'MixedProcessor', 'MixedItemList'
]


def _decode(df):
    return np.array(
        [[df.columns[i] for i, t in enumerate(x) if t == 1]
         for x in df.values],
        dtype=np.object)
...
def _get_files(parent, p, f, extensions):
    ...
    res = [
        p / o for o in f if not o.startswith('.') and
        (extensions is None or f'.{o.split(".")[-1].lower()}' in low_extensions
         )
    ]
    return res


def get_files(path: PathOrStr,
...
class ItemList():
    ...
    def __init__(self,
        ...
        self.label_cls, self.inner_df, self.processor = ifnone(
            label_cls, self._label_cls), inner_df, processor
    ...
    def reconstruct(self, t: Tensor, x: Tensor = None):
        ...
        return self[0].reconstruct(t, x) if has_arg(
            self[0].reconstruct, 'x') else self[0].reconstruct(t)

    def new(self, items: Iterator, processor: PreProcessors = None,
            **kwargs) -> 'ItemList':
    ...
    def __getitem__(self, idxs: int) -> Any:
        ...
        if isinstance(idxs, Integral): return self.get(idxs)
            ...
            return self.new(
                self.items[idxs], inner_df=index_row(self.inner_df, idxs))
    ...
    def from_df(cls,
        ...
        res = cls(
            items=_maybe_squeeze(inputs.values),
            path=path,
            inner_df=df,
            processor=processor,
            **kwargs)
    ...
    def from_csv(cls,
        ...
        df = pd.read_csv(
            Path(path) / csv_name, delimiter=delimiter, header=header)
        return cls.from_df(
            df, path=path, cols=cols, processor=processor, **kwargs)
    ...
    def use_partial_data(self, sample_pct: float = 0.01,
                         seed: int = None) -> 'ItemList':
    ...
    def filter_by_folder(self, include=None, exclude=None):
        ...
        def _inner(o):
            if isinstance(o, Path): n = o.relative_to(self.path).parts[0]
                ...
                n = o.split(os.path.sep)[len(
                    str(self.path).split(os.path.sep))]
    ...
    def _get_by_folder(self, name):
        return [
            i for i in range_of(self)
            if (self.items[i].parts[self.num_parts] if isinstance(
                self.items[i], Path) else self.items[i].split(os.path.sep)[0]
                ) == name
        ]

    def split_by_folder(self, train: str = 'train',
                        valid: str = 'valid') -> 'ItemLists':
    ...
    def random_split_by_pct(self, valid_pct: float = 0.2, seed: int = None):
        warn(
            "`random_split_by_pct` is deprecated, please use `split_by_rand_pct`."
        )
        return self.split_by_rand_pct(valid_pct=valid_pct, seed=seed)

    def split_by_rand_pct(self, valid_pct: float = 0.2,
                          seed: int = None) -> 'ItemLists':
    ...
    def split_subsets(self, train_size: float, valid_size: float,
                      seed=None) -> 'ItemLists':
    ...
    def split_by_fname_file(self, fname: PathOrStr,
                            path: PathOrStr = None) -> 'ItemLists':
    ...
    def split_from_df(self, col: IntsOrStrs = 2):
        ...
        valid_idx = np.where(
            self.inner_df.iloc[:, df_names_to_idx(col, self.inner_df)])[0]
        return self.split_by_idx(valid_idx)
    ...
    def label_from_df(self,
                      cols: IntsOrStrs = 1,
                      label_cls: Callable = None,
                      **kwargs):
        "Label `self.items` from the values in `cols` in `self.inner_df`."
        labels = self.inner_df.iloc[:, df_names_to_idx(cols, self.inner_df)]
        assert labels.isna().sum().sum(
        ) == 0, f"You have NaN values in column(s) {cols} of your dataframe, please fix it."
        if is_listy(cols) and len(cols) > 1 and (
                label_cls is None or label_cls == MultiCategoryList):
            new_kwargs, label_cls = dict(
                one_hot=True, classes=cols), MultiCategoryList
            kwargs = {**new_kwargs, **kwargs}
        return self._label_from_list(
            _maybe_squeeze(labels), label_cls=label_cls, **kwargs)

    def label_const(self, const: Any = 0, label_cls: Callable = None,
                    **kwargs) -> 'LabelList':
        "Label every item with `const`."
        return self.label_from_func(
            func=lambda o: const, label_cls=label_cls, **kwargs)
    ...
    def label_from_folder(self, label_cls: Callable = None,
                          **kwargs) -> 'LabelList':
        "Give a label to each filename depending on its folder."
        return self.label_from_func(
            func=
            lambda o: (o.parts if isinstance(o, Path) else o.split(os.path.sep))[-2],
            label_cls=label_cls,
            **kwargs)

    def label_from_re(self,
                      pat: str,
...
class EmptyLabelList(ItemList):
    ...
    def reconstruct(self, t: Tensor, x: Tensor = None):
        ...
        return self.x.reconstruct(t, x) if has_arg(
            self.x.reconstruct, 'x') else self.x.reconstruct(t)
...
class MultiCategoryProcessor(CategoryProcessor):
    ...
    def process_one(self, item):
        ...
        res = [
            super(MultiCategoryProcessor, self).process_one(o) for o in item
        ]
...
class MultiCategoryList(CategoryListBase):
    ...
    def get(self, i):
        ...
        return MultiCategory(
            one_hot(o, self.c), [self.classes[p] for p in o], o)
...
class ItemLists():
    ...
    def __init__(self, path: PathOrStr, train: ItemList, valid: ItemList):
        self.path, self.train, self.valid, self.test = Path(
            path), train, valid, None
    ...
    def __dir__(self) -> List[str]:
        ...
        add_ons = [
            'label_const', 'label_empty', 'label_from_df', 'label_from_folder',
            'label_from_func', 'label_from_list', 'label_from_re'
        ]
    ...
    def transform(self,
                  tfms: Optional[Tuple[TfmList, TfmList]] = (None, None),
                  **kwargs):
        "Set `tfms` to be applied to the xs of the train and validation set."
        if not tfms: tfms = (None, None)
        assert is_listy(tfms) and len(
            tfms
        ) == 2, "Please pass a list of two lists of transforms (train and valid)."
...
class LabelLists(ItemLists):
    ...
    def get_processors(self):
        ...
        xp = ifnone(
            self.train.x.processor, [p(ds=self.train.x) for p in procs_x])
        yp = ifnone(
            self.train.y.processor, [p(ds=self.train.y) for p in procs_y])
        return xp, yp
    ...
    def databunch(self,
        ...
        data = self.x._bunch.create(
            ...
            no_check=no_check,
            **kwargs)
        if getattr(self, 'normalize',
                   False):  #In case a normalization was serialized
            norm = self.normalize
            data.normalize(
                (norm['mean'], norm['std']),
                do_x=norm['do_x'],
    ...
    def add_test(self, items: Iterator, label: Any = None):
        ...
        if isinstance(items, ItemList):
            items = self.valid.x.new(
                items.items, inner_df=items.inner_df).process()
        else:
...
class LabelList(Dataset):
    ...
    def new(self, x, y, **kwargs) -> 'LabelList':
        if isinstance(x, ItemList):
            return self.__class__(
                x, y, tfms=self.tfms, tfm_y=self.tfm_y, **self.tfmargs)
        else:
            return self.new(self.x.new(x, **kwargs),
                            self.y.new(y, **kwargs)).process()
    ...
    def __getitem__(self, idxs: Union[int, np.ndarray]) -> 'LabelList':
        ...
        if isinstance(idxs, Integral):
            ...
            if hasattr(self, 'tfms_y') and self.tfm_y and self.item is None:
                y = y.apply_tfms(self.tfms_y, **{
                    **self.tfmargs_y, 'do_resolve': False
                })
            if y is None: y = 0
            return x, y
        else:
    ...
    def load_state(cls, path: PathOrStr, state: dict) -> 'LabelList':
        ...
        x = state['x_cls'](
            [], path=path, processor=state['x_proc'], ignore_empty=True)
        y = state['y_cls'](
            [], path=path, processor=state['y_proc'], ignore_empty=True)
        res = cls(
            x, y, tfms=state['tfms'], tfm_y=state['tfm_y'],
            **state['tfmargs']).process()
    ...
    def transform(self, tfms: TfmList, tfm_y: bool = None, **kwargs):
        ...
        tfms_y = None if tfms is None else list(
            filter(lambda t: t.use_on_y, listify(tfms)))
...
class MixedProcessor(PreProcessor):
    def __init__(
            self,
            procs: Collection[Union[PreProcessor, Collection[PreProcessor]]]):
        self.procs = procs
...
class MixedItem(ItemBase):
    ...
    def __repr__(self):
        return '\n'.join([f'{self.__class__.__name__}'] +
                         [repr(item) for item in self.obj])

    def apply_tfms(self, tfms: Collection, **kwargs):
        self.obj = [
            item.apply_tfms(t, **kwargs) for item, t in zip(self.obj, tfms)
        ]
...
class MixedItemList(ItemList):
    def __init__(self,
        ...
        if processor is None:
            default_procs = [[p(ds=il) for p in listify(il._processor)]
                             for il in item_lists]
            processor = MixedProcessor([
                ifnone(il.processor, dp)
                for il, dp in zip(item_lists, default_procs)
            ])
    ...
    def new(self, item_lists, processor: PreProcessor = None,
            **kwargs) -> 'ItemList':
        "Create a new `ItemList` from `items`, keeping the same attributes."
        processor = ifnone(processor, self.processor)
        copy_d = {o: getattr(self, o) for o in self.copy_new}
    ...
    def __getitem__(self, idxs: int) -> Any:
        ...
        if isinstance(idxs, Integral): return self.get(idxs)
            ...
            return self.new(
                item_lists, inner_df=index_row(self.inner_df, idxs))
