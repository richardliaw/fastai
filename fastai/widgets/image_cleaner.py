...
class DatasetFormatter():
    ...
    def get_toplosses_idxs(cls, learn, n_imgs, **kwargs):
        ...
        _, _, top_losses = learn.get_preds(
            ds_type=DatasetType.Fix, with_loss=True)
        idxs = torch.topk(top_losses, n_imgs)[1]
        return cls.padded_ds(dl.dataset, **kwargs), idxs

    def padded_ds(ll_input,
                  size=(250, 300),
...
class ImageCleaner():
    ...
    def create_image_list(self, dataset, fns_idxs):
        ...
        if self._duplicates:
            ...
            chunked_idxs = [
                chunk for chunk in chunked_idxs
                if Path(items[chunk[0]]).is_file()
                and Path(items[chunk[1]]).is_file()
            ]
            return [(dataset.x[i]._repr_jpeg_(), items[i],
                     self._labels[dataset.y[i].data]) for chunk in chunked_idxs
                    for i in chunk]
        else:
            return [(dataset.x[i]._repr_jpeg_(), items[i],
                     self._labels[dataset.y[i].data]) for i in fns_idxs
                    if Path(items[i]).is_file()]
    ...
    def get_widgets(self, duplicates):
        ...
        widgets = []
        for (img, fp,
             human_readable_label) in self._all_images[:self._batch_size]:
            img_widget = self.make_img_widget(
                img, layout=Layout(height='250px', width='300px'))
            dropdown = self.make_dropdown_widget(
                description='',
                options=self._labels,
                value=human_readable_label,
                file_path=fp,
                handler=self.relabel,
                layout=Layout(width='auto'))
            delete_btn = self.make_button_widget(
                'Delete', file_path=fp, handler=self.on_delete)
            widgets.append(
                self.make_vertical_box(
                    [img_widget, dropdown, delete_btn],
                    layout=Layout(
                        width='auto', height='300px', overflow_x="hidden"),
    ...
    def batch_contains_deleted(self):
        ...
        imgs = [
            self._all_images[:self._batch_size][0][1],
            self._all_images[:self._batch_size][1][1]
        ]
    ...
    def render(self):
        ...
        if self.batch_contains_deleted():
            ...
            display(
                self.make_horizontal_box(self.get_widgets(self._duplicates)))
            display(
