...
class ImageDownloader():
    ...
    def _init_ui(self) -> VBox:
        ...
        self._preview_header = widgets.HTML(
            self._heading, layout=Layout(height='60px'))
...
def download_google_images(path: PathOrStr,
                           search_term: str,
                           size: str = '>400*300',
                           n_images: int = 10,
                           format: str = 'jpg',
                           max_workers: int = defaults.cpus,
                           timeout: int = 4) -> FilePathList:
    """
...
def _search_url(search_term: str, size: str = '>400*300',
                format: str = 'jpg') -> str:
    ...
    return ('https://www.google.com/search?q=' + quote(search_term) +
            ...
            _url_params(
                size, format) + '&sa=X&ei=XosDVaCXD8TasATItgE&ved=0CAcQ_AUoAg')
...
def _fetch_img_tuples(url: str, format: str = 'jpg',
                      n_images: int = 10) -> list:
...
def _html_to_img_tuples(html: str, format: str = 'jpg',
                        n_images: int = 10) -> list:
    ...
    img_tuples = ((_img_fname(d['ou']), d['ou']) for d in metadata_dicts
                  if d['ity'] == format)
    return list(itertools.islice(img_tuples, n_images))


def _fetch_img_tuples_webdriver(url: str,
                                format: str = 'jpg',
                                n_images: int = 150) -> list:
    """
    Parse the Google Images Search for urls and return the image metadata as tuples (fname, url).
...
def _download_images(label_path: PathOrStr,
                     img_tuples: list,
                     max_workers: int = defaults.cpus,
                     timeout: int = 4) -> FilePathList:
    """
...
def _download_single_image(label_path: Path,
                           img_tuple: tuple,
                           i: int,
                           timeout: int = 4) -> None:
    """
