...
__all__ = [
    'AverageMetric', 'Callback', 'CallbackHandler', 'OptimWrapper',
    'SmoothenValue', 'Scheduler', 'annealing_cos', 'CallbackList',
...
class OptimWrapper():
    ...
    def create(cls,
               ...
               wd: Floats = 0.,
               true_wd: bool = False,
               bn_wd: bool = True) -> optim.Optimizer:
    ...
    def new_with_params(self,
                        param_groups: Collection[Collection[nn.Parameter]]):
        "Create a new `OptimWrapper` from `self` with another `layer_groups` but the same hyper-parameters."
        opt_func = getattr(self, 'opt_func', self.opt.__class__)
        opt = opt_func([{'params': p, 'lr': 0} for p in param_groups])
        opt = self.__class__(
            opt, wd=self.wd, true_wd=self.true_wd, bn_wd=self.bn_wd)
    ...
    def read_defaults(self) -> None:
        ...
        reserved_names = [
            'params', 'lr', 'momentum', 'alpha', 'betas', 'weight_decay'
        ]
    ...
    def set_stat(self, name: str,
                 value: Union[float, Collection[float]]) -> None:
    ...
    @classmethod
    def load_with_state_and_layer_group(cls, state: dict,
                                        layer_groups: Collection[nn.Module]):
        res = cls.create(
            state['opt_func'],
            state['lr'],
...
class Callback():
    ...
    def get_state(self, minimal: bool = True):
        ...
        to_remove = ['exclude', 'not_min'] + getattr(self, 'exclude',
                                                     []).copy()
    ...
    def __repr__(self):
        ...
        list_repr = [self.__class__.__name__] + [
            f'{k}: {getattr(self, k)}' for k in attrs
...
class CallbackHandler():
    ...
    def __post_init__(self) -> None:
        ...
        self.metrics = [(met
                         if isinstance(met, Callback) else AverageMetric(met))
                        for met in self.metrics]
        self.callbacks = sorted(
            self.callbacks, key=lambda o: getattr(o, '_order', 0))
        self.smoothener = SmoothenValue(self.beta)
        self.state_dict: Dict[str, Union[int, float,
                                         Tensor]] = _get_init_state()

    def _call_and_update(self, cb, cb_name, **kwargs) -> None:
        "Call `cb_name` on `cb` and update the inner state."
        new = ifnone(
            getattr(cb, f'on_{cb_name}')(**self.state_dict, **kwargs), dict())
    ...
    def on_train_begin(self, epochs: int, pbar: PBar,
                       metrics: MetricFuncList) -> None:
        "About to start learning."
        self.state_dict = _get_init_state()
        self.state_dict.update(
            dict(n_epochs=epochs, pbar=pbar, metrics=metrics))
        names = [(met.name if hasattr(met, 'name') else
                  camel2snake(met.__class__.__name__)) for met in self.metrics]
    ...
    def on_epoch_begin(self) -> None:
        "Handle new epoch."
        self.state_dict['num_batch'], self.state_dict[
            'stop_training'] = 0, False
        self('epoch_begin')

    def on_batch_begin(self, xb: Tensor, yb: Tensor,
    ...
    def on_backward_begin(self, loss: Tensor) -> None:
        ...
        self.state_dict['last_loss'], self.state_dict[
            'smooth_loss'] = loss, self.smoothener.smooth
...
class AverageMetric(Callback):
    ...
    def __init__(self, func):
        ...
        name = func.__name__ if hasattr(func,
                                        '__name__') else func.func.__name__
...
def do_annealing_poly(start: Number, end: Number, pct: float,
...
class Scheduler():
    ...
    def __init__(self,
        ...
        self.start, self.end = (vals[0], vals[1]) if is_tuple(vals) else (vals,
                                                                          0)
