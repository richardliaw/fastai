...
__all__ = [
    'add_datepart', 'cont_cat_split', 'Categorify', 'FillMissing',
    'FillStrategy', 'Normalize', 'TabularProc', 'add_elapsed_times',
    'make_date', 'add_cyclic_datepart'
]
...
def cyclic_dt_feat_names(time: bool = True,
                         add_linear: bool = False) -> List[str]:
    "Return feature names of date/time cycles as produced by `cyclic_dt_features`."
    fs = ['cos', 'sin']
    attr = [
        f'{r}_{f}' for r in 'weekday day_month month_year day_year'.split()
        for f in fs
    ]
    if time:
...
def cyclic_dt_features(d: Union[date, datetime],
                       time: bool = True,
                       add_linear: bool = False) -> List[float]:
    ...
    if add_linear:
        if type(d) == date: feats.append(d.year + rs[-1])
            ...
            secs_in_year = (datetime(d.year + 1, 1, 1) - datetime(
                d.year, 1, 1)).total_seconds()
            feats.append(d.year + (
                (d - datetime(d.year, 1, 1)).total_seconds() / secs_in_year))
    return feats
...
def _get_elapsed(df: DataFrame, field_names: Collection[str], date_field: str,
                 base_field: str, prefix: str):
    for f in field_names:
        day1 = np.timedelta64(1, 'D')
        last_date, last_base, res = np.datetime64(), None, []
        for b, v, d in zip(df[base_field].values, df[f].values,
                           df[date_field].values):
...
def add_elapsed_times(df: DataFrame, field_names: Collection[str],
                      date_field: str, base_field: str):
    ...
    work_df = _get_elapsed(work_df, field_names, date_field, base_field,
                           'After')
    work_df = work_df.sort_values(
        [base_field, date_field], ascending=[True, False])
    work_df = _get_elapsed(work_df, field_names, date_field, base_field,
                           'Before')

    for a in ['After' + f
              for f in field_names] + ['Before' + f for f in field_names]:
...
def cont_cat_split(df, max_card=20, dep_var=None) -> Tuple[List, List]:
    ...
    for label in df:
        ...
        if df[label].dtype == int and df[label].unique(
        ).shape[0] > max_card or df[label].dtype == float:
