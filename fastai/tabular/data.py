...
__all__ = [
    'TabularDataBunch', 'TabularLine', 'TabularList', 'TabularProcessor',
    'tabular_learner'
]
...
class TabularDataBunch(DataBunch):
    ...
    def from_df(cls,
        ...
        cont_names = ifnone(cont_names,
                            list(set(df) - set(cat_names) - {dep_var}))
        procs = listify(procs)
        ...
        src = src.label_from_df(
            cols=dep_var) if classes is None else src.label_from_df(
                cols=dep_var, classes=classes)
...
class TabularList(ItemList):
    ...
    def get(self, o):
        if not self.preprocessed:
            return self.inner_df.iloc[o] if hasattr(
                self, 'inner_df') else self.items[o]
...
def tabular_learner(data: DataBunch,
