...
__all__ = [
    *tabular.__all__, 'EmbeddingDotBias', 'EmbeddingNN', 'collab_learner',
    'CollabDataBunch', 'CollabLine', 'CollabList', 'CollabLearner'
]
...
class EmbeddingDotBias(nn.Module):
    ...
    def __init__(self,
        ...
        (self.u_weight, self.i_weight, self.u_bias, self.i_bias) = [
            embedding(*o)
            for o in [(n_users, n_factors), (n_items,
                                             n_factors), (n_users,
                                                          1), (n_items, 1)]
        ]
    ...
    def forward(self, users: LongTensor, items: LongTensor) -> Tensor:
        ...
        res = dot.sum(1) + self.u_bias(users).squeeze() + self.i_bias(
            items).squeeze()
        if self.y_range is None: return res
        return torch.sigmoid(res) * (
            self.y_range[1] - self.y_range[0]) + self.y_range[0]
...
class CollabDataBunch(DataBunch):
    ...
    def from_df(cls,
                ...
                no_check: bool = False) -> 'CollabDataBunch':
        "Create a `DataBunch` suitable for collaborative filtering from `ratings`."
        user_name = ifnone(user_name, ratings.columns[0])
        item_name = ifnone(item_name, ratings.columns[1])
        rating_name = ifnone(rating_name, ratings.columns[2])
        cat_names = [user_name, item_name]
        src = (CollabList.from_df(
            ratings, cat_names=cat_names, procs=Categorify).split_by_rand_pct(
                valid_pct=valid_pct,
                seed=seed).label_from_df(cols=rating_name))
        if test is not None:
