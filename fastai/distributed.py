...
__all__ = [
    'DistributedRecorder', 'DistributedTrainer', 'read_metrics',
    'setup_distrib'
]
...
class DistributedTrainer(LearnerCallback):
    ...
    def on_train_end(self, **kwargs):
        ...
        if hasattr(self.learn.data,
                   'valid_dl') and self.learn.data.valid_dl is not None:
...
class DistributedRecorder(LearnerCallback):
    ...
    def save_stats(self):
        ...
        np.save(cache_path / f'losses_{self.cuda_id}',
                np.array(recorder.losses))
        stats = np.array(
...
def _learner_distributed(learn: Learner,
                         cuda_id: int,
                         cache_dir: PathOrStr = 'tmp'):
