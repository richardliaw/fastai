...
__all__ = [
    'PIL', 'Image', 'ImageBBox', 'ImageSegment', 'ImagePoints', 'FlowField',
    'RandTransform', 'TfmAffine', 'TfmCoord', 'TfmCrop', 'TfmLighting',
    'TfmPixel', 'Transform', 'bb2hw', 'image2np', 'open_image', 'open_mask',
    'tis2hw', 'pil2tensor', 'scale_flow', 'show_image', 'CoordFunc', 'TfmList',
    'open_mask_rle', 'rle_encode', 'rle_decode', 'ResizeMethod', 'plot_flat',
    'plot_multi', 'show_multi', 'show_all'
]
...
class Image(ItemBase):
    ...
    def apply_tfms(self,
        ...
        tfms = listify(tfms)
        xtra = ifnone(xtra, {})
        default_rsz = ResizeMethod.SQUISH if (
            size is not None and is_listy(size)) else ResizeMethod.CROP
        ...
        x = self.clone()
        x.set_sample(
            padding_mode=padding_mode, mode=mode, remove_out=remove_out)
        if size is not None:
    ...
    def show(self,
        ...
        ax = show_image(
            self, ax=ax, hide_axis=hide_axis, cmap=cmap, figsize=figsize)
...
class ImageSegment(Image):
    ...
    def lighting(self, func: LightingFunc, *args: Any,
                 **kwargs: Any) -> 'Image':
        return self
...
class ImagePoints(Image):
    ...
    def clone(self):
        ...
        return self.__class__(
            FlowField(self.size, self.flow.flow.clone()),
            scale=False,
            y_first=False)
    ...
    def show(self,
        ...
        pnt = scale_flow(
            FlowField(self.size, self.data), to_unit=False).flow.flip(1)
...
class ImageBBox(ImagePoints):
    ...
    def __init__(self,
        ...
        if labels is not None and len(labels) > 0 and not isinstance(
                labels[0], Category):
    ...
    def create(cls,
        ...
        tr_corners = torch.cat([bboxes[:, 0][:, None], bboxes[:, 3][:, None]],
                               1)
        bl_corners = bboxes[:, 1:3].flip(1)
        bboxes = torch.cat(
            [bboxes[:, :2], tr_corners, bl_corners, bboxes[:, 2:]], 1)
    ...
    def _compute_boxes(self) -> Tuple[LongTensor, LongTensor]:
        bboxes = self.flow.flow.flip(1).view(-1, 4, 2).contiguous().clamp(
            min=-1, max=1)
        mins, maxes = bboxes.min(dim=1)[0], bboxes.max(dim=1)[0]
        bboxes = torch.cat([mins, maxes], 1)
        mask = (bboxes[:, 2] - bboxes[:, 0] > 0) * (bboxes[:, 3] - bboxes[:, 1]
                                                    > 0)
        if len(mask) == 0:
...
def _remove_points_out(flow: FlowField):
    pad_mask = (flow.flow[:, 0] >= -1) * (flow.flow[:, 0] <= 1) * (
...
class Transform():
    ...
    def __init__(self, func: Callable, order: Optional[int] = None):
        ...
        self.func.__name__ = func.__name__[
            1:]  #To remove the _ that begins every transform function.
...
class RandTransform():
    ...
    def __call__(self, x: Image, *args, **kwargs) -> Image:
        ...
        return self.tfm(x, *args, **{
            **self.resolved,
            **kwargs
        }) if self.do_run else x
...
def _grid_sample(x: TensorImage,
                 coords: FlowField,
                 mode: str = 'bilinear',
                 padding_mode: str = 'reflection',
                 remove_out: bool = True) -> TensorImage:
    "Resample pixels in `coords` from `x` by `mode`, with `padding_mode` in ('reflection','border','zeros')."
    coords = coords.flow.permute(0, 3, 1, 2).contiguous().permute(
        0, 2, 3, 1)  # optimize layout for grid_sample
    ...
    return F.grid_sample(
        x[None], coords, mode=mode, padding_mode=padding_mode)[0]
...
def _get_crop_target(target_px: Union[int, TensorImageSize],
                     mult: int = None) -> Tuple[int, int]:
...
def _get_resize_target(img, crop_target, do_crop=False) -> TensorImageSize:
    ...
    return ch, int(round(r / ratio)), int(
        round(c / ratio)
...
def plot_multi(func: Callable[[int, int, plt.Axes], None],
               r: int = 1,
               c: int = 1,
               figsize: Tuple = (12, 6)):
...
def show_multi(func: Callable[[int, int], Image],
               r: int = 1,
               c: int = 1,
               figsize: Tuple = (9, 9)):
...
def show_all(imgs: Collection[Image],
             r: int = 1,
             c: Optional[int] = None,
             figsize=(12, 6)):
