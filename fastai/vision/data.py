...
__all__ = [
    'get_image_files', 'denormalize', 'get_annotations', 'ImageDataBunch',
    'ImageList', 'normalize', 'normalize_funcs', 'resize_to', 'channel_view',
    'mnist_stats', 'cifar_stats', 'imagenet_stats', 'download_images',
    'verify_images', 'bb_pad_collate', 'ImageImageList', 'PointsLabelList',
    'ObjectCategoryList', 'ObjectItemList', 'SegmentationLabelList',
    'SegmentationItemList', 'PointsItemList'
]
...
def get_image_files(c: PathOrStr, check_ext: bool = True,
                    recurse=False) -> FilePathList:
    "Return list of files in `c` that are images. `check_ext` will filter to `image_extensions`."
    return get_files(
        c,
        extensions=(image_extensions if check_ext else None),
        recurse=recurse)
...
def get_annotations(fname, prefix=None):
    ...
    id2images, id2bboxes, id2cats = {}, collections.defaultdict(
    ...
    return [id2images[k] for k in ids], [[id2bboxes[k], id2cats[k]]
                                         for k in ids]
...
def normalize(x: TensorImage, mean: FloatTensor,
              std: FloatTensor) -> TensorImage:
    "Normalize `x` with `mean` and `std`."
    return (x - mean[..., None, None]) / std[..., None, None]


def denormalize(x: TensorImage,
                mean: FloatTensor,
                std: FloatTensor,
                do_x: bool = True) -> TensorImage:
    "Denormalize `x` with `mean` and `std`."
    return x.cpu().float(
    ) * std[..., None, None] + mean[..., None, None] if do_x else x.cpu()


def _normalize_batch(b: Tuple[Tensor, Tensor],
                     mean: FloatTensor,
                     std: FloatTensor,
                     do_x: bool = True,
...
def normalize_funcs(mean: FloatTensor,
                    std: FloatTensor,
                    do_x: bool = True,
    ...
    return (partial(
        _normalize_batch, mean=mean, std=std, do_x=do_x, do_y=do_y),
...
class ImageDataBunch(DataBunch):
    ...
    def from_df(cls,
        ...
        src = (ImageList.from_df(
            df, path=path, folder=folder, suffix=suffix, cols=fn_col)
               .split_by_rand_pct(valid_pct).label_from_df(
                   label_delim=label_delim, cols=label_col))
        return cls.create_from_ll(src, **kwargs)
    ...
    @staticmethod
    def single_from_classes(path: Union[Path, str],
        ...
        return sd.label_const(
            0, label_cls=CategoryList, classes=classes).transform(
                ds_tfms, **kwargs).databunch()
...
def download_images(urls: Collection[str],
                    dest: PathOrStr,
                    max_pics: int = 1000,
                    max_workers: int = 8,
                    timeout=4):
...
def verify_image(file: Path,
    ...
    try:
        ...
        if (max_size is not None and
            (img.height > max_size
             or img.width > max_size)) or img_channels != n_channels:
            assert isinstance(
                dest,
                Path), "You should provide `dest` Path to save resized image"
...
def verify_images(path: PathOrStr,
                  delete: bool = True,
                  max_workers: int = 4,
...
class ImageList(ItemList):
    ...
    def from_df(cls,
        ...
        res.items = np.char.add(
            np.char.add(pref, res.items.astype(str)), suffix)
        return res
...
def _ll_pre_transform(self, train_tfm: List[Callable],
                      valid_tfm: List[Callable]):
...
def _db_pre_transform(self, train_tfm: List[Callable],
                      valid_tfm: List[Callable]):
    "Call `train_tfm` and `valid_tfm` after opening image, before converting from `PIL.Image`"
