...
__all__ = [
    'basic_critic', 'basic_generator', 'GANModule', 'GANLoss', 'GANTrainer',
    'FixedGANSwitcher', 'AdaptiveGANSwitcher', 'GANLearner', 'NoisyItem',
    'GANItemList', 'gan_critic', 'AdaptiveLoss', 'accuracy_thresh_expand',
    'GANDiscriminativeLR'
]
...
def basic_critic(in_size: int,
    ...
    while cur_size > 4:
        layers.append(
            conv_layer(
                cur_ftrs, cur_ftrs * 2, 4, 2, 1, leaky=0.2, **conv_kwargs))
        cur_ftrs *= 2
        cur_size //= 2
...
def basic_generator(in_size: int,
                    n_channels: int,
                    noise_sz: int = 100,
                    n_features: int = 64,
                    n_extra_layers=0,
    ...
    while cur_size < in_size:
        ...
        cur_ftrs *= 2
    layers = [
        conv_layer(noise_sz, cur_ftrs, 4, 1, transpose=True, **conv_kwargs)
    ]
    cur_size = 4
    ...
    layers += [
        conv2d_trans(cur_ftrs, n_channels, 4, 2, 1, bias=False),
        nn.Tanh()
    ]
    return nn.Sequential(*layers)
...
class GANLoss(GANModule):
    ...
    def __init__(self, loss_funcG: Callable, loss_funcC: Callable,
                 gan_model: GANModule):
        super().__init__()
    ...
    def critic(self, real_pred, input):
        ...
        fake = self.gan_model.generator(
            input.requires_grad_(False)).requires_grad_(True)
...
class GANTrainer(LearnerCallback):
    ...
    def on_train_begin(self, **kwargs):
        ...
        if not getattr(self, 'opt_gen', None):
            self.opt_gen = self.opt.new(
                [nn.Sequential(*flatten_model(self.generator))])
        else:
            self.opt_gen.lr, self.opt_gen.wd = self.opt.lr, self.opt.wd
        if not getattr(self, 'opt_critic', None):
            self.opt_critic = self.opt.new(
                [nn.Sequential(*flatten_model(self.critic))])
        else:
            self.opt_critic.lr, self.opt_critic.wd = self.opt.lr, self.opt.wd
        self.gen_mode = self.gen_first
        self.switch(self.gen_mode)
        self.closses, self.glosses = [], []
        self.smoothenerG, self.smoothenerC = SmoothenValue(
            self.beta), SmoothenValue(self.beta)
...
def gan_loss_from_func(loss_gen,
    ...
    def _loss_G(fake_pred, output, target, weights_gen=weights_gen):
        ...
        return weights_gen[0] * loss_crit(
            fake_pred, ones) + weights_gen[1] * loss_gen(output, target)
...
class GANLearner(Learner):
    ...
    def __init__(self,
        ...
        switcher = ifnone(switcher, partial(
            FixedGANSwitcher, n_crit=5, n_gen=1))
        super().__init__(
            data,
            gan,
            loss_func=loss_func,
            callback_fns=[switcher],
            **learn_kwargs)
        trainer = GANTrainer(
...
def gan_critic(n_channels: int = 3,
               nf: int = 128,
               n_blocks: int = 3,
               p: int = 0.15):
...
def accuracy_thresh_expand(y_pred: Tensor,
                           y_true: Tensor,
                           thresh: float = 0.5,
                           sigmoid: bool = True) -> Rank0Tensor:
    "Compute accuracy after expanding `y_true` to the size of `y_pred`."
    if sigmoid: y_pred = y_pred.sigmoid()
    return ((y_pred > thresh) == y_true[:, None].expand_as(y_pred).byte()
            ).float().mean()
