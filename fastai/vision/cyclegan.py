...
def pad_conv_norm_relu(
        ...
        stride: int = 1,
        activ: bool = True,
        init: Callable = nn.init.kaiming_normal_) -> List[nn.Module]:
    layers = []
    ...
    conv = nn.Conv2d(
        ch_in, ch_out, kernel_size=ks, padding=p, stride=stride, bias=bias)
    if init:
        init(conv.weight)
...
class ResnetBlock(nn.Module):
    def __init__(self,
                 ...
                 bias: bool = True):
        super().__init__()
        assert pad_mode in ['zeros', 'reflection',
                            'border'], f'padding {pad_mode} not implemented.'
...
def resnet_generator(ch_in: int,
                     ch_out: int,
                     n_ftrs: int = 64,
                     norm_layer: nn.Module = None,
                     dropout: float = 0.,
                     n_blocks: int = 6,
                     pad_mode: str = 'reflection') -> nn.Module:
    norm_layer = ifnone(norm_layer, nn.InstanceNorm2d)
...
class CycleGAN(nn.Module):
    def __init__(self,
        ...
        self.D_B = critic(
            ch_in, n_features, disc_layers, norm_layer, sigmoid=not lsgan)
        self.G_A = resnet_generator(ch_in, ch_out, n_features, norm_layer,
                                    drop, gen_blocks)
        self.G_B = resnet_generator(ch_in, ch_out, n_features, norm_layer,
                                    drop, gen_blocks)
...
class AdaptiveLoss(nn.Module):
    ...
    def forward(self, output, target: bool):
        targ = output.new_ones(*output.size()) if target else output.new_zeros(
            *output.size())
...
class CycleGanLoss(nn.Module):
    def __init__(self,
        ...
        self.crit = AdaptiveLoss(F.mse_loss
                                 if lsgan else F.binary_cross_entropy)
    ...
    def forward(self, output, target):
        ...
        idt_loss = self.l_idt * (self.l_B * F.l1_loss(idt_A, self.real_B) +
...
class CycleGANTrainer(LearnerCallback):
    ...
    def on_train_begin(self, **kwargs):
        ...
        self.opt_D_A = self.learn.opt.new(
            [nn.Sequential(*flatten_model(self.D_A))])
        self.opt_D_B = self.learn.opt.new(
            [nn.Sequential(*flatten_model(self.D_B))])
    ...
    def on_batch_end(self, last_input, last_output, **kwargs):
        ...
        self.D_A.zero_grad()
        loss_D_A = 0.5 * (self.crit(self.D_A(real_A), True) + self.crit(
            self.D_A(fake_A), False))
        loss_D_A.backward()
        self.opt_D_A.step()
        self._set_trainable(D_B=True)
        self.D_B.zero_grad()
        loss_D_B = 0.5 * (self.crit(self.D_B(real_B), True) + self.crit(
            self.D_B(fake_B), False))
        loss_D_B.backward()
        self.opt_D_B.step()
        self._set_trainable()
    ...
    def on_epoch_end(self, last_metrics, **kwargs):
        ...
        return add_metrics(last_metrics,
                           [s.smooth for k, s in self.smootheners.items()])
