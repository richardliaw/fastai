...
__all__ = [
    'brightness', 'contrast', 'crop', 'crop_pad', 'cutout', 'dihedral',
...
def _rotate(degrees: uniform):
    ...
    return [[cos(angle), -sin(angle), 0.], [sin(angle),
                                            cos(angle), 0.], [0., 0., 1.]]
...
def _zoom(scale: uniform = 1.0, row_pct: uniform = 0.5,
          col_pct: uniform = 0.5):
...
def _dihedral_affine(k: partial(uniform_int, 0, 7)):
    ...
    x = -1 if k & 1 else 1
    y = -1 if k & 2 else 1
    if k & 4: return [[0, x, 0.], [y, 0, 0], [0, 0, 1.]]
...
def _pad_coord(x, row_pad: int, col_pad: int, mode='zeros'):
    ...
    x.flow = FlowField((h + 2 * row_pad, w + 2 * col_pad),
                       x.flow.flow * pad[None])
    return x
...
def _crop_image_points(x, size, row_pct=0.5, col_pct=0.5):
    ...
    x.flow.flow.add_(-1 + torch.Tensor(
        [w / cols - 2 * col / cols, h / rows - 2 * row / rows])[None])
    x.size = (rows, cols)
    return x


def _crop(x, size, row_pct: uniform = 0.5, col_pct: uniform = 0.5):
    f_crop = _crop_image_points if isinstance(x,
                                              ImagePoints) else _crop_default
...
def _crop_pad_default(x,
    ...
    if x.size(1) < rows or x.size(2) < cols:
        ...
        x = F.pad(
            x[None], (col_pad, col_pad, row_pad, row_pad),
            mode=padding_mode)[0]
    row = int((x.size(1) - rows + 1) * row_pct)
    col = int((x.size(2) - cols + 1) * col_pct)
...
def _crop_pad(x,
              ...
              row_pct: uniform = 0.5,
              col_pct: uniform = 0.5):
    f_crop_pad = _crop_pad_image_points if isinstance(
        x, ImagePoints) else _crop_pad_default
...
def _perspective_warp(c, magnitude: partial(uniform, size=8) = 0,
                      invert=False):
    "Apply warp of `magnitude` to `c`."
    magnitude = magnitude.view(4, 2)
    targ_pts = [[x + m for x, m in zip(xs, ms)]
                for xs, ms in zip(_orig_pts, magnitude)]
...
def _symmetric_warp(c, magnitude: partial(uniform, size=4) = 0, invert=False):
    ...
    targ_pts = [[-1 - m[3], -1 - m[1]], [-1 - m[2], 1 + m[1]],
                [1 + m[3], -1 - m[0]], [1 + m[2], 1 + m[0]]]
    return _do_perspective_warp(c, targ_pts, invert)
...
def _tilt(c, direction: uniform_int, magnitude: uniform = 0, invert=False):
    ...
    coeffs = _find_coeffs(targ_pts, _orig_pts) if invert else _find_coeffs(
        _orig_pts, targ_pts)
...
def get_transforms(do_flip: bool = True,
...
def _compute_zs_mat(sz: TensorImageSize, scale: float, squish: float,
                    invert: bool, row_pct: float,
                    col_pct: float) -> AffineMatrix:
...
def rand_resize_crop(size: int,
                     max_scale: float = 2.,
                     ratios: Tuple[float, float] = (0.75, 1.33)):
