...
__all__ = [
    'PResNet', 'presnet18', 'presnet34', 'presnet50', 'presnet101',
    'presnet152'
]
...
def conv(ni, nf, ks=3, stride=1, bias=False):
    return nn.Conv2d(
        ni, nf, kernel_size=ks, stride=stride, padding=ks // 2, bias=bias)
...
class PResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000):
        ...
        self.avgpool = nn.Sequential(act_fn(), nn.BatchNorm2d(ni),
...
def presnet18(pretrained=False, **kwargs):
    return presnet(
        BasicBlock, [2, 2, 2, 2], 'presnet18', pre=pretrained, **kwargs)


def presnet34(pretrained=False, **kwargs):
    return presnet(
        BasicBlock, [3, 4, 6, 3], 'presnet34', pre=pretrained, **kwargs)


def presnet50(pretrained=False, **kwargs):
    return presnet(
        Bottleneck, [3, 4, 6, 3], 'presnet50', pre=pretrained, **kwargs)


def presnet101(pretrained=False, **kwargs):
    return presnet(
        Bottleneck, [3, 4, 23, 3], 'presnet101', pre=pretrained, **kwargs)


def presnet152(pretrained=False, **kwargs):
    return presnet(
        Bottleneck, [3, 8, 36, 3], 'presnet152', pre=pretrained, **kwargs)
