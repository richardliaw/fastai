...
__all__ = [
    'XResNet', 'xresnet18', 'xresnet34', 'xresnet50', 'xresnet101',
    'xresnet152'
]
...
class XResNet(nn.Sequential):
    def __init__(self, expansion, layers, c_in=3, c_out=1000):
        ...
        blocks = [
            self._make_layer(expansion, block_szs[i], block_szs[i + 1], l, 1
                             if i == 0 else 2) for i, l in enumerate(layers)
        ]
        super().__init__(
            *stem,
    ...
    def _make_layer(self, expansion, ni, nf, blocks, stride):
        return nn.Sequential(*[
            ResBlock(expansion, ni if i == 0 else nf, nf, stride
                     if i == 0 else 1) for i in range(blocks)
        ])
