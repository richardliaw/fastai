...
__all__ = [
    'cnn_learner', 'create_cnn', 'create_cnn_model', 'create_body',
...
def cnn_learner(data: DataBunch,
                base_arch: Callable,
                cut: Union[int, Callable] = None,
                pretrained: bool = True,
                lin_ftrs: Optional[Collection[int]] = None,
...
@classmethod
def _cl_int_from_learner(cls,
                         learn: Learner,
                         ds_type: DatasetType = DatasetType.Valid,
                         tta=False):
    "Create an instance of `ClassificationInterpretation`. `tta` indicates if we want to use Test Time Augmentation."
    preds = learn.TTA(
        ds_type=ds_type, with_loss=True) if tta else learn.get_preds(
...
def _cl_int_plot_top_losses(self,
                            k,
                            largest=True,
                            figsize=(12, 12),
                            heatmap: bool = None,
...
def _cl_int_plot_multi_top_losses(self,
                                  samples: int = 3,
    ...
    for i, pred in enumerate(predclass):
        ...
        if l_dim > 1:
            infotup = (i, pred, where_truth, losses[i][pred],
                       np.round(self.probs[i], decimals=3)[pred], mismatch)
        else:
            infotup = (i, pred, where_truth, losses[i],
                       np.round(self.probs[i], decimals=3)[pred], mismatch)
...
def _learner_interpret(learn: Learner,
    ...
    return ClassificationInterpretation.from_learner(
        learn, ds_type=ds_type, tta=tta)
