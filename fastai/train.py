...
__all__ = [
    'BnFreeze', 'GradientClipping', 'ShowGraph',
    'ClassificationInterpretation', 'fit_one_cycle', 'lr_find',
...
class ShowGraph(LearnerCallback):
    ...
    def on_epoch_end(self, n_epochs: int, last_metrics: MetricsList,
                     **kwargs) -> bool:
        ...
        if last_metrics is not None and np.any(last_metrics):
            ...
            x_bounds = (0,
                        (n_epochs - len(rec.nb_batches)) * rec.nb_batches[-1] +
                        len(rec.losses))
            y_bounds = (0,
                        max((max(Tensor(rec.losses)),
                             max(Tensor(rec.val_losses)))))
            rec.pbar.update_graph([(iters, rec.losses),
                                   (val_iter, rec.val_losses)], x_bounds,
                                  y_bounds)
        return {}
...
class AccumulateScheduler(LearnerCallback):
    ...
    def on_train_begin(self, **kwargs):
        ...
        if hasattr(self.loss_func,
                   "reduction") and (self.loss_func.reduction != "sum"):
...
class ClassificationInterpretation():
    ...
    def confusion_matrix(self, slice_size: int = 1):
        ...
        if slice_size is None:
            cm = ((self.pred_class == x[:, None]) &
                  (self.y_true == x[:, None, None])).sum(2)
        else:
    ...
    def most_confused(self, min_val: int = 1,
                      slice_size: int = 1) -> Collection[Tuple[str, str, int]]:
