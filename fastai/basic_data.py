...
class DeviceDataLoader():
    ...
    def batch_size(self, v):
        new_kwargs = {
            **self.dl.init_kwargs, 'batch_size': v,
    ...
    def new(self, **kwargs):
        ...
        return DeviceDataLoader(
            self.dl.__class__(self.dl.dataset, **new_kwargs), self.device,
            self.tfms, self.collate_fn)
...
class DataBunch():
    ...
    def __init__(self,
        ...
        def _create_dl(dl, **kwargs):
            ...
            return DeviceDataLoader(dl, self.device, self.dl_tfms, collate_fn,
                                    **kwargs)
    ...
    @staticmethod
    def _init_ds(train_ds: Dataset,
                 valid_ds: Dataset,
                 test_ds: Optional[Dataset] = None):
        # train_ds, but without training tfms
        fix_ds = valid_ds.new(train_ds.x, train_ds.y) if hasattr(
            valid_ds, 'new') else train_ds
        return [
            o for o in (train_ds, valid_ds, fix_ds, test_ds) if o is not None
        ]
    ...
    def create(cls,
        ...
        dls = [
            DataLoader(
                ...
                num_workers=num_workers,
                **dl_kwargs)
            for d, b, s in zip(datasets, (bs, val_bs, val_bs,
                                          val_bs), (True, False, False, False))
            if d is not None
        ]
        return cls(
            *dls,
    ...
    def dl(self, ds_type: DatasetType = DatasetType.Valid) -> DeviceDataLoader:
        ...
        #TODO: refactor
        return (self.train_dl if ds_type == DatasetType.Train else self.test_dl
                if ds_type == DatasetType.Test else self.valid_dl
                if ds_type == DatasetType.Valid else self.single_dl
                if ds_type == DatasetType.Single else self.fix_dl)
    ...
    def add_test(self, items: Iterator, label: Any = None) -> None:
        ...
        self.test_dl = DeviceDataLoader(dl, vdl.device, vdl.tfms,
                                        vdl.collate_fn)
    ...
    def show_batch(self,
        ...
        xs = [
            self.train_ds.x.reconstruct(grab_idx(x, i)) for i in range(n_items)
        ]
        ...
        if has_arg(self.train_ds.y.reconstruct, 'x'):
            ...
            ys = [
                self.train_ds.y.reconstruct(grab_idx(y, i))
    ...
    def export(self, file: PathLikeOrBinaryStream = 'export.pkl'):
        ...
        xtra = dict(normalize=self.norm.keywords) if getattr(
            self, 'norm', False) else {}
    ...
    def loss_func(self) -> OptLossFunc:
        return getattr(self.train_ds.y, 'loss_func', F.nll_loss) if hasattr(
            self.train_ds, 'y') else F.nll_loss

    @property
    def test_ds(self) -> Dataset:
        return self._grab_dataset(
            self.test_dl) if self.test_dl is not None else None
    ...
    def sanity_check(self):
        ...
        if not hasattr(self.train_ds, 'items') or len(
                self.train_ds.items) == 0 or not hasattr(
                    self.train_dl, 'batch_sampler'):
            return
