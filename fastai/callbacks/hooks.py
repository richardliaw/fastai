...
__all__ = [
    'ActivationStats', 'Hook', 'HookCallback', 'Hooks', 'hook_output',
    'hook_outputs', 'model_sizes', 'num_features_model', 'model_summary',
    'dummy_eval', 'dummy_batch'
]
...
def hook_output(module: nn.Module, detach: bool = True,
                grad: bool = False) -> Hook:
    "Return a `Hook` that stores activations of `module` in `self.stored`"
    return Hook(module, _hook_inner, detach=detach, is_forward=not grad)


def hook_outputs(modules: Collection[nn.Module],
                 detach: bool = True,
                 grad: bool = False) -> Hooks:
...
class HookCallback(LearnerCallback):
    ...
    def on_train_begin(self, **kwargs):
        ...
        if not self.modules:
            self.modules = [
                m for m in flatten_model(self.learn.model)
                if hasattr(m, 'weight')
            ]
...
class ActivationStats(HookCallback):
    ...
    def hook(self, m: nn.Module, i: Tensors,
             o: Tensors) -> Tuple[Rank0Tensor, Rank0Tensor]:
...
def dummy_batch(m: nn.Module, size: tuple = (64, 64)) -> Tensor:
    ...
    ch_in = in_channels(m)
    return one_param(m).new(1, ch_in, *size).requires_grad_(False).uniform_(
        -1., 1.)
...
def params_size(m: Union[nn.Module, Learner],
                size: tuple = (3, 64, 64)) -> Tuple[Sizes, Tensor, Hooks]:
    ...
    if isinstance(m, Learner):
        ...
        ds_type = DatasetType.Train if m.data.train_dl else (DatasetType.Valid
                                                             if m.data.valid_dl
                                                             else
                                                             DatasetType.Test)
    ...
    with hook_outputs(flatten_model(m)) as hook_o:
        with hook_params(flatten_model(m)) as hook_p:
            ...
            output_size = [((o.stored.shape[1:])
                            if o.stored is not None else None) for o in hook_o]
            params = [(o.stored if o.stored is not None else (None, None))
                      for o in hook_p]
...
def layers_info(m: Collection[nn.Module]) -> Collection[namedtuple]:
    ...
    layer_info = namedtuple('Layer_Information',
                            ['Layer', 'OutputSize', 'Params', 'Trainable'])
    return list(
        map(layer_info, layers_names, layers_sizes, layers_params,
            layers_trainable))
