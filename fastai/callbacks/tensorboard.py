...
__all__ = [
    'LearnerTensorboardWriter', 'GANTensorboardWriter',
...
class LearnerTensorboardWriter(LearnerCallback):
    ...
    def on_train_begin(self, **kwargs: Any) -> None:
        self.graph_writer.write(
            model=self.learn.model,
            tbwriter=self.tbwriter,
            input_to_model=next(iter(self.learn.data.dl(
                DatasetType.Single)))[0])

    def on_batch_end(self, last_loss: Tensor, iteration: int,
                     **kwargs) -> None:
    ...
    def on_epoch_end(self, last_metrics: MetricsList, iteration: int,
                     **kwargs) -> None:
...
class ModelImageSet():
    ...
    @staticmethod
    def get_list_from_model(learn: Learner, ds_type: DatasetType,
                            batch: Tuple) -> []:
        "Factory method to convert a batch of model images to a list of ModelImageSet."
        image_sets = []
        x, y = batch[0], batch[1]
        preds = learn.pred_batch(
            ds_type=ds_type, batch=(x, y), reconstruct=True)
...
class HistogramTBRequest(TBWriteRequest):
    ...
    def __init__(self, model: nn.Module, iteration: int,
                 tbwriter: SummaryWriter, name: str):
        super().__init__(tbwriter=tbwriter, iteration=iteration)
        self.params = [(name, values.clone().detach().cpu())
    ...
    def _write_histogram(self, param_name: str, values) -> None:
        ...
        self.tbwriter.add_histogram(
            tag=tag, values=values, global_step=self.iteration)
...
class ModelStatsTBRequest(TBWriteRequest):
    ...
    def __init__(self, model: nn.Module, iteration: int,
        ...
        super().__init__(tbwriter=tbwriter, iteration=iteration)
        self.gradients = [
            x.grad.clone().detach().cpu() for x in model.parameters()
    ...
    def _write_avg_gradient(self) -> None:
        ...
        avg_gradient = sum(x.data.mean()
                           for x in self.gradients) / len(self.gradients)
    ...
    def _write_median_gradient(self) -> None:
        ...
        median_gradient = statistics.median(
            x.data.median() for x in self.gradients)
        self._add_gradient_scalar(
            'median_gradient', scalar_value=median_gradient)
...
class ImageTBRequest(TBWriteRequest):
    ...
    def __init__(self, learn: Learner, batch: Tuple, iteration: int,
                 tbwriter: SummaryWriter, ds_type: DatasetType):
        super().__init__(tbwriter=tbwriter, iteration=iteration)
...
class ImageTBWriter():
    ...
    def write(self, learn: Learner, trn_batch: Tuple, val_batch: Tuple,
              iteration: int, tbwriter: SummaryWriter) -> None:
    ...
    def _write_for_dstype(self, learn: Learner, batch: Tuple, iteration: int,
                          tbwriter: SummaryWriter,
                          ds_type: DatasetType) -> None:
...
class GraphTBRequest(TBWriteRequest):
    ...
    def __init__(self, model: nn.Module, tbwriter: SummaryWriter,
...
class GraphTBWriter():
    ...
    def write(self, model: nn.Module, tbwriter: SummaryWriter,
