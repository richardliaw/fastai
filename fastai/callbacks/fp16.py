...
def get_master(layer_groups: ModuleList, flat_master: bool = False
               ) -> Tuple[List[List[Tensor]], List[List[Tensor]]]:
    "Return two lists, one for the model parameters in FP16 and one for the master parameters in FP32."
    split_params = split_no_wd_params(layer_groups)
    model_params = [[param for param in pg if param.requires_grad]
                    for pg in split_params]
    if flat_master:
        master_params = []
...
def model_g2master_g(model_params: Sequence[Tensor],
...
def master2model(model_params: Sequence[Tensor],
                 master_params: Sequence[Tensor],
                 flat_master: bool = False) -> None:
...
class MixedPrecision(LearnerCallback):
    ...
    def on_backward_begin(self, last_loss: Rank0Tensor,
                          **kwargs: Any) -> Rank0Tensor:
    ...
    def on_backward_end(self, **kwargs: Any) -> None:
        ...
        if self.dynamic and grad_overflow(
                self.model_params) and self.loss_scale > 1:
            self.loss_scale /= 2
            self.noskip = 0
